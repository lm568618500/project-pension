 ...
 */
public class UnionPayModifyDto implements Serializable {

	private static final long serialVersionUID = 1572423719073L;
	 ...
	 * 金额单位元
	 */
	private java.lang.Long cod;
	/**
	 * 使用方式: 1.qrtype=qr 扫码 支付使用; 2.qrtype=h5 手机 网站调起支付宝 微信支付(咨询 大华技术人员进 行使用);3. qrtype=gzh在 微信公众号内进 行支付; 4.qrtype=app 商 户 app 集成银商app 支付插件， 调用微信支付宝 云闪付支付5. qrtype=uac银联全渠道,无卡 支付和企业网银 6.qrtype=xcx 微 信小程序支付;
	 */
	private java.lang.String qrtype;
	/**
	 * 支付方式非必填，不填时直接跳转相应支付页面。 当指定支付方式 97，98，94 时一般是程 序已经运行在相应的 app 内的公众号支 付和 h5 支付时需要唤醒哪种 app 而使用 97:微信,
98:支付宝, 94:云闪付
	 */
	...
	/**
	 * 分账编码 选填，实时分账使用，大华约定该编码 来配置对应分账的商户号，自动计算分 账金额(如果使用该编码进行分账，不 需要传递 subOrders 和 platformAmount)
	 */
	private java.lang.String dscode;
	/**
	 * 分账编码 (分账方案二)选填，此电商分账方式与 pos 机的分账 规则一样，只适合 2 方分账(支持全分， 按定比例分，按数量分，按固定金额分)， 如果商户的分账金额涉及到商户系统的 积分、折扣等，又不能使用平台和子商 户关联分账，可以使用 fee 参数，商户自 行计算分账金额
	 */
	private java.lang.String bizCode;
	/**
	 * 分账金额 (适合电商分账， 银商业务)非必填,单位元，保留 2 位小数，选填 根据 bizCode 配置的商户的分账金额，传 递该参数后,实际支付金额就是 cod 和 fee 之和
	 */
	private java.lang.String fee;
	/**
	 * 选填，分账使用，不分账不用传递，
[{‘mid’:’123456789012345’,’totalAmount ’:’200’}]，其中totalAmount的单位是分， 所 有 的 totalAmount+ platformAmount=cod*100，中间涉及到的 单位商户可以自行处理一下(如果使用 subOrders 商户自己传递分账商户明细， platformAmount 必传，dscode 不传递)
	 */
	...
	/**
	 *  非必填,证件类型 微信支持身份证、支付宝支持身份证: IDENTITY_CARD、 护照:PASSPORT、 军官证:OFFICER_CARD、 士兵证:SOLDIER_CARD、 户口本:HOKOU
	 */
	...
	/**
	 * 异步通知 如果商户只存在一个 erp 系统，异步通知 地址可以提供给大华运营人员配置在大 华后台，无需每笔交易都传递;如果商 户根据不同的订单通知的接口地址不一 样，需要传递 notifyUrl 参数。如果在大 华后台配置了通知地址，下单也传递了 notifyUrl 参数后，该笔订单支付成功后， 只会通知到 notifyUrl 参数中的接口地址 上
	 */
	private java.lang.String notifyUrl;
	/**
	 * 申请微信支付 商户 App，调用微信小程序支付必传
appid
	 */
	...
	public void setCod(java.lang.Long cod) {
		this.cod = cod;
	}

	public java.lang.Long getCod() {
		return this.cod;
	}
