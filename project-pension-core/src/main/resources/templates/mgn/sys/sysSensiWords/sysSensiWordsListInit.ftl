    ...
    <form class="layui-form">
        <div class="layui-inline" style="margin-left: 15px">
            ...
            <div class="layui-input-inline">
                <input type="text" name="c_words" lay-verify="title" autocomplete="off" placeholder="请输入敏感词" class="layui-input">
            </div>
        </div>
...
<script>
    layui.use(['layer','form','table'], function() {
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sys//sysSensiWords/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys//sysSensiWords/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys//sysSensiWords/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sys/sysSensiWords/sysSensiWordsShowInit?id=" + data.id,
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "编辑敏感词",
                    type : 2,
                    content : "${serverPath}/mgn/sys/sysSensiWords/sysSensiWordsEditInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回敏感词列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
                            });
                        },500);
                    }
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "添加敏感词",
                    type : 2,
                    content : "${serverPath}/mgn/sys/sysSensiWords/sysSensiWordsAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回敏感词列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
