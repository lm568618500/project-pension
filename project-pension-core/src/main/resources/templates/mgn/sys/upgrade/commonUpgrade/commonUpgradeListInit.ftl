    ...
    <form class="layui-form">
        <div class="layui-inline" style="margin-left: 15px">
            ...
            <div class="layui-input-inline">
                <input type="text" name="c_version" lay-verify="title" autocomplete="off" placeholder="请输入版本名称" class="layui-input">
            </div>
        </div>
...
<script type="text/html" id="isNeedTpl">
	{{#  if(d.isNeed === 0){ }}
		<span class="layui-badge layui-bg-red">否</span>
	{{#  } else if(d.isNeed === 1){ }}
		<span class="layui-badge layui-bg-blue">是</span>
...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					//{field:'adminId', title: '操作者ID'},
					{field:'appName', title: 'App'},
					{field:'version', title: '版本名称'},
					//{field:'tips', title: '更新提示'},
					{field:'pubTime', title: '发布时间'},
					//{field:'platForm', title: '平台'},
					//{field:'downType', title: '下载类型， 1、本地 2、第三方'},
					//{field:'fileName', title: '文件名'},
					{field:'downUrl', title: '下载地址'},
					//{field:'qrCode', title: '二维码',templet:'<div><img src="{{ d.qrCode}}"></div>'},
					{field:'isNeed', title: '强制更新',templet: '#isNeedTpl'},
					{field:'isDefault', title: '默认版本',templet: '#defaultTpl'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sys/upgrade/commonUpgrade/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/upgrade/commonUpgrade/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/upgrade/commonUpgrade/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sys/upgrade/commonUpgrade/commonUpgradeShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/upgrade/commonUpgrade/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sys/upgrade/commonUpgrade/commonUpgradeEditInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sys/upgrade/commonUpgrade/commonUpgradeAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
