    ...
    <form class="layui-form">
        <div class="layui-inline" style="margin-left: 15px">
            ...
            <div class="layui-input-inline">
            	<select name="setId"  lay-filter="firstSettingSelect">
			        <option value="">全部</option>
			        <option value="1">花型</option>
        ...
        <div class="layui-inline" style="margin-left: 15px">
            <label>分类名称:</label>
            <div class="layui-input-inline">
        	     <input type="text" name="setName" id="setName" placeholder="请输入分类名称" autocomplete="off" class="layui-input">
            </div>
        </div>
...
<script type="text/html" id="thumb">
  <img src='{{d.resHref}}' height="25" style="border-radius: 50%;" />
</script>
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					{field:'setName', title: '归类名称'},
					{field:'resName', title: '文件名称'},
					{field:'resHref', title: '图片' ,toolbar: '#thumb'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sys/ossResStore/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/ossResStore/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/ossResStore/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sys/ossResStore/ossResStoreShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/ossResStore/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "编辑数据",
                    type : 2,
                    content : "${serverPath}/mgn/sys/ossResStore/ossResStoreEditInit?id=" + data.id,
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "新增数据",
                    type : 2,
                    content : "${serverPath}/mgn/sys/ossResStore/ossResStoreAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
