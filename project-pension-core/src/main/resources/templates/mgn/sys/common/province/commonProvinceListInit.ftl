    ...
    <form class="layui-form">
        <div class="layui-inline" style="margin-left: 15px">
            ...
            <div class="layui-input-inline">
                <input type="text" name="c_name" lay-verify="title" autocomplete="off" placeholder="请输入省份名称" class="layui-input">
            </div>
        </div>
...
<script>
    layui.use(['layer','form','table'], function() {
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sys/common/province/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/common/province/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/common/province/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sys/common/province/commonProvinceShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/common/province/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sys/common/province/commonProvinceEditInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sys/common/province/commonProvinceAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
