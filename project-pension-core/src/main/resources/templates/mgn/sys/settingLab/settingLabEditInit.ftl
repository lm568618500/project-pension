...
<form class="layui-form" style="width:80%;">
        <input type="hidden" name="id" lay-verify="required" value="${view.id!}" class="layui-input">
        <input type="hidden"  id="contact" value="${view.contact!}" name="contact">
        <input type="hidden"  id="otherContact" value="${view.otherContact!}" name="otherContact">
    	<div class="layui-form-item">
	        <label class="layui-form-label"><span style="color:red">*</span><@i18N key='sys.settingLab.ShiYanShiMingCheng' /></label>
	        <div class="layui-input-block">
	        	<input type="text" name="name" lay-verify="required" value="${view.name!}"  autocomplete="off" placeholder="<@i18N key='qsr' /><@i18N key='sys.settingLab.ShiYanShiMingCheng' />" class="layui-input">
	        </div>
    	</div>
    	<div class="layui-form-item">
	        <label class="layui-form-label"><span style="color:red">*</span><@i18N key='sys.settingLab.JianChaFanWei' /></label>
	        <div class="layui-input-block">
	        	<input type="text" name="scope" lay-verify="required" value="${view.scope!}" autocomplete="off" placeholder="<@i18N key='qsr' /><@i18N key='sys.settingLab.JianChaFanWei' />" class="layui-input">
	        </div>
    	</div>
    	<div class="layui-form-item">
	        <label class="layui-form-label"><span style="color:red">*</span><@i18N key='sys.settingLab.XiangXiDiZhi' /></label>
	        <div class="layui-input-block">
	        	<input type="text" name="address" lay-verify="required" value="${view.address!}" autocomplete="off" placeholder="<@i18N key='qsr' /><@i18N key='sys.settingLab.XiangXiDiZhi' />" class="layui-input">
	        </div>
    	</div>
    	<div class="layui-form-item">
		    <label class="layui-form-label"><span style="color:red">*</span><@i18N key='status' /></label>
		    <div class="layui-input-block">
		      <input type="radio" name="status" value="1" title="<@i18N key='admin.kq' />" <#if view.status == 1> checked=""</#if> >
		      <input type="radio" name="status" value="0" title="<@i18N key='evaluation.ty' />" <#if view.status == 0> checked=""</#if>>
		    </div>
		</div>
		<div class="layui-form-item">
			...
			<div class="layui-input-block" id="contactText">
			    <button type="button" id="contactButton" class="layui-btn layui-btn-primary layui-btn-sm">${adminView.realName}</button>
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-block" id="otherContactText" >
			    <#list admins as admin>
			      <button type="button" onclick="delHtml(this)"class="layui-btn layui-btn-primary layui-btn-sm">${admin.realName}<i class="layui-icon"></i>
			           <input type="hidden"   id="otherContactId" name="otherContactId" value="${admin.id}">
			      </button>
			    </#list>
    	...
    	<div class="layui-form-item">
	        ...
	        <div class="layui-input-block">
                 <textarea placeholder="请输入内容" name="mark" class="layui-textarea">${view.mark!}</textarea>
            </div>
    	</div>
		<div class="layui-form-item">
        <div class="layui-input-block">
    ...
    layui.use(['form','jquery','layer','laydate','layedit'],function(){
       var form = layui.form,
        ...
        form.on("submit(modify)",function(data){
            ...
            $.post("${serverPath}/mgn/sys/settinglab/modify",data.field,function(res){
               layer.close(loadIndex);
               if(res.result){
