    ...
    <form class="layui-form">
        <div class="layui-inline" style="margin-left: 15px">
            ...
            <div class="layui-input-inline">
                <input type="text" name="c_resName" lay-verify="title" autocomplete="off" placeholder="请输入资源名称" class="layui-input">
            </div>
        </div>
        <!--
         <div class="layui-inline" style="margin-left: 15px">
...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					{field:'resSize', title: '资源大小'},
					{field:'suffix', title: '文件后缀'},
					//{field:'ossType', title: '存储类型(1/本地FASTDFS 2/七牛OSS 3/阿里云OSS)'},
					{field:'fileHash', title: '文件Hash'},
					{field:'ip', title: 'IP'},
					//{field:'group', title: 'FASTDFS组'},
					//{field:'remoteResPath', title: '远程地址'},
					//{field:'remoteResFullPath', title: '远程全地址'},
					{field:'preViewPath', title: '预览地址'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sys/oss/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/oss/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sys/oss/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sys/oss/ossResource/ossResourceShowInit?id=" + data.id,
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sys/oss/ossResource/ossResourceEditInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sys/oss/ossResource/ossResourceAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
