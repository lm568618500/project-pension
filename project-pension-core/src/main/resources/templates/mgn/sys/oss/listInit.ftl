...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
                ...
                {field:'remoteResFullPath', title: '地址'}
            ];
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该日志么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/admin/system/log/delete",{"ids":[data.id]},function (res){
                           if(res.success){
                ...
                if(data.length > 0){
                    layer.confirm("你确定要删除这些日志么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            ...
                            $.post("${serverPath}/admin/system/log/delete",{ids:d},function (res) {
                                if(res.success){
                                    layer.msg("删除成功",{time: 1000},function(){
                                        table.reload('test', t);
                                    });
