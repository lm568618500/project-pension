...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					{field:'orderType', title: '订单类型:1-支付,2-转账,3-退款'},
					{field:'notifyUrl', title: '通知地址'},
					{field:'notifyCount', title: '通知次数'},
					{field:'result', title: '通知响应结果'},
					{field:'lastupdate', title: ''},
					{field:'status', title: '通知状态,1-通知中,2-通知成功,3-通知失败'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sm/pay/payMchNotify/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/pay/payMchNotify/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/pay/payMchNotify/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sm/pay/payMchNotify/payMchNotifyShowInit?id=" + data.id,
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sm/pay/payMchNotify/payMchNotifyEditInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sm/pay/payMchNotify/payMchNotifyAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
