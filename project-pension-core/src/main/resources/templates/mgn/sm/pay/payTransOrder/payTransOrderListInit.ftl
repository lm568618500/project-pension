...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					{field:'status', title: '转账状态:0-订单生成,1-转账中,2-转账成功,3-转账失败,4-业务处理完成'},
					{field:'isDel', title: ''},
					{field:'transOrderId', title: '转账订单号'},
					{field:'mchId', title: '商户ID'},
					{field:'mchTransNo', title: '商户转账单号'},
					{field:'channelId', title: '渠道ID'},
					{field:'channelItemsId', title: '支付方式'},
					{field:'amount', title: '转账金额,单位分'},
					{field:'currency', title: '三位货币代码,人民币:cny'},
					{field:'result', title: '转账结果:0-不确认结果,1-等待手动处理,2-确认成功,3-确认失败'},
					...
					{field:'transSuccTime', title: '订单转账成功时间'},
					{field:'buzNotifyUrl', title: ''},
					{field:'buzNotifyResp', title: ''},
					{field:'buzStatus', title: ''},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sm/pay/payTransOrder/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/pay/payTransOrder/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/pay/payTransOrder/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sm/pay/payTransOrder/payTransOrderShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/pay/payTransOrder/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "编辑数据",
                    type : 2,
                    content : "${serverPath}/mgn/sm/pay/payTransOrder/payTransOrderEditInit?id=" + data.id,
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "新增数据",
                    type : 2,
                    content : "${serverPath}/mgn/sm/pay/payTransOrder/payTransOrderAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
