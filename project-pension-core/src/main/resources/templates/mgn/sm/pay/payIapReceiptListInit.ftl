...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					{field:'status', title: '处理状态:0-未处理,1-处理成功,-1-处理失败'},
					{field:'isDel', title: ''},
					{field:'payOrderId', title: '支付订单号'},
					{field:'mchId', title: '商户ID'},
					{field:'transactionId', title: 'IAP业务号'},
					{field:'receiptData', title: '渠道ID'},
					{field:'handleCount', title: '处理次数'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sm/pay/payIapReceipt/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/pay/payIapReceipt/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/pay/payIapReceipt/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sm/pay/payIapReceipt/payIapReceiptShowInit?id=" + data.id,
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sm/pay/payIapReceipt/payIapReceiptEditInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sm/pay/payIapReceipt/payIapReceiptAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
