...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					{field:'goodsorderid', title: '商品订单ID'},
					{field:'goodsid', title: '商品ID'},
					{field:'goodsname', title: '商品名称'},
					{field:'amount', title: '金额,单位分'},
					{field:'userid', title: '用户ID'},
					{field:'status', title: '订单状态,订单生成(0),支付成功(1),处理完成(2),处理失败(-1)'},
					{field:'payorderid', title: '支付订单号'},
					{field:'channelid', title: '渠道ID'},
					{field:'channeluserid', title: '支付渠道用户ID(微信openID或支付宝账号等第三方支付账号)'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sm/pay/payGoodsOrder/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/pay/payGoodsOrder/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/pay/payGoodsOrder/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sm/pay/payGoodsOrder/payGoodsOrderShowInit?id=" + data.id,
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sm/pay/payGoodsOrder/payGoodsOrderEditInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/sm/pay/payGoodsOrder/payGoodsOrderAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
