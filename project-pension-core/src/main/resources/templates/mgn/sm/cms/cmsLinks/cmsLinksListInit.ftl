    ...
    <form class="layui-form">
        <div class="layui-inline" style="margin-left: 15px">
            ...
            <div class="layui-input-inline">
                <input type="text" name="c_linkName" lay-verify="title" autocomplete="off" placeholder="请输入伙伴名称" class="layui-input">
            </div>
        </div>
         ...
         <div class="layui-inline" style="margin-left: 15px">
            <label></label>
            <div class="layui-input-inline">
        	      <select name="c_linkType" >
			        <option value="">类型</option>
			        <option value="1">友情连接</option>
...
<script type="text/html" id="linkTypeTpl">
	{{#  if(d.linkType === 1){ }}
		<span class="layui-badge layui-bg-red">友情连接</span>
	{{#  } else if(d.linkType === 2){ }}
		<span class="layui-badge layui-bg-blue">合作伙伴</span>
	{{#  } else { }}
		<span class="layui-badge layui-bg-gray">其它</span>
	{{#  } }}
</script>
...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					//{field:'linkImg', title: '图片地址'},
					//{field:'linkHref', title: '超链接'},
					//{field:'linkSort', title: '排序'},
					{field:'linkType', title: '类型',  templet: '#linkTypeTpl'},
					//{field:'isRecommend', title: '是否推荐到首页'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sm/cms/cmsLinks/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/cms/cmsLinks/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/cms/cmsLinks/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sm/cms/cmsLinks/cmsLinksShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/cms/cmsLinks/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "编辑数据",
                    type : 2,
                    content : "${serverPath}/mgn/sm/cms/cmsLinks/cmsLinksEditInit?id=" + data.id,
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "新增数据",
                    type : 2,
                    content : "${serverPath}/mgn/sm/cms/cmsLinks/cmsLinksAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
