...
<script type="text/html" id="operation">
	...
	{{#  if(d.isIndex === 0){ }}
		<a class="layui-btn layui-btn-xs" lay-event="toIndex">首页推荐</a>
	{{#  } else if(d.isIndex === 1){ }}
		<a class="layui-btn layui-btn-xs" lay-event="toUnIndex">取消首页推荐</a>
	{{#  } else { }}
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "编辑数据",
                    type : 2,
                    content : "${serverPath}/mgn/sm/cms/cmsContent/cmsContentEditInit?id=" + data.id,
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/cms/cmsContent/del",{"id":data.id},function (res){
                           if(res.result.ret){
            ...
            if(obj.event === "toIndex"){
                layer.confirm("你确定要将文章推荐至首页么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/cms/cmsContent/toIndex",{"id":data.id},function (res){
                           if(res.result.ret){
             ...
             if(obj.event === "toUnIndex"){
                layer.confirm("你确定要将文章取消推荐么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/cms/cmsContent/toUnIndex",{"id":data.id},function (res){
                           if(res.result.ret){
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "新增数据",
                    type : 2,
                    content : "${serverPath}//mgn/sm/cms/cmsContent/cmsContentAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
