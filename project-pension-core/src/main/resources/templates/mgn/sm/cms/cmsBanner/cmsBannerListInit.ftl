...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					{field:'id', title: ''},
					{field:'locationId', title: 't_cms_banner_location id'},
					{field:'title', title: '说明'},
					{field:'urls', title: '跳转地址'},
					{field:'images', title: 'banner图'},
					{field:'bSort', title: '排序'},
					{field:'iswai', title: '是否外链（1，是，2，否）'},
					{field:'adminid', title: '操作用户'},
					{field:'time', title: '创建时间'},
					{field:'lastupdate', title: '修改时间'},
					{field:'status', title: '是否启用（1是 0否）'},
					{field:'isDel', title: '是否删除(0否，-1是)'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/sm/cms/cmsBanner/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/cms/cmsBanner/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/cms/cmsBanner/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/sm/cms/cmsBanner/cmsBannerShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/sm/cms/cmsBanner/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "编辑数据",
                    type : 2,
                    content : "${serverPath}/mgn/sm/cms/cmsBanner/cmsBannerEditInit?id=" + data.id,
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "新增数据",
                    type : 2,
                    content : "${serverPath}/mgn/sm/cms/cmsBanner/cmsBannerAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
