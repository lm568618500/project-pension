...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					{field:'isDel', title: '删除状态 -1删除 0正常'},
					{field:'mechName', title: '商户名称'},
					{field:'supportSms', title: '短信支持1支持 0不支持'},
					{field:'supportEmail', title: '邮件支持1支持 0不支持'},
					{field:'supportPush', title: '推送支持1支持 0不支持'},
					{field:'amount', title: '余额 分 。暂不使用。后续支持'},
					{field:'lockStatus', title: '锁定状态 1锁定 0正常'},
					{field:'lockStart', title: '锁定开始时间'},
					{field:'lockEnd', title: '锁定结束时间'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/message/messageMech/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/message/messageMech/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/message/messageMech/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/message/messageMech/messageMechShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/message/messageMech/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/message/messageMech/messageMechEditInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/message/messageMech/messageMechAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
