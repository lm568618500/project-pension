    ...
    <form class="layui-form">
        <div class="layui-inline" style="margin-left: 15px">
            ...
            <div class="layui-input-inline">
                <input type="text" name="c_tempName" lay-verify="title" autocomplete="off" placeholder="请输入模板名称" class="layui-input">
            </div>
        </div>
        ...
        <div class="layui-inline" style="margin-left: 15px">
            <label></label>
            <div class="layui-input-inline">
                <input type="text" name="c_otherTemplateId" lay-verify="title" autocomplete="off" placeholder="请输入第三方模板ID" class="layui-input">
            </div>
        </div>
        ...
        <div class="layui-inline" style="margin-left: 15px">
            <label></label>
            <div class="layui-input-inline">
                <input type="text" name="c_smsContent" lay-verify="title" autocomplete="off" placeholder="请输入短信内容" class="layui-input">
            </div>
        </div>
...
<script type="text/html" id="immediatelyTpl">
	{{#  if(d.immediately === 0){ }}
		<span class="layui-badge layui-bg-red">否</span>
	{{#  } else if(d.immediately === 1){ }}
...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					{field:'tempName', title: '模板名称'},
					{field:'otherTemplateId', title: '第三方模板ID'},
					{field:'smsContent', title: '短信内容'},
					//{field:'status', title: '短信开关0关闭 1开启'},
					{field:'immediately', title: '立即发送' ,templet: '#immediatelyTpl' },
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/message/messageTempSms/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/message/messageTempSms/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/message/messageTempSms/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/message/messageTempSms/messageTempSmsShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/message/messageTempSms/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "编辑短信模板",
                    type : 2,
                    content : "${serverPath}/mgn/message/messageTempSms/messageTempSmsEditInit?id=" + data.id,
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "发布任务",
                    type : 2,
                    content : "${serverPath}/mgn/message/messageTempSms/messageTempSmsAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
