...
<form class="layui-form" style="width:80%;">
		<div class="layui-form-item" >
			...
			<div class="layui-input-inline">
				<input type="hidden" name="id" lay-verify="title" autocomplete="off" value="${view.id!}" placeholder="请输入id" class="layui-input">
				<select name="oneParentId" lay-verify="required" lay-filter="oneSelect">
					<option value>全部</option>
					<#list oneList as oneList>
						<option value="${oneList.id!}" <#if view.oneParentId==oneList.id>selected</#if>>${oneList.name!}</option>
					</#list>
				</select>
			</div>
		</div>
		<div class="layui-form-item" >
			...
			<div class="layui-input-inline">
				<select name="twoParentId" id="parentTwoId">
					<option value>全部</option>
					<#list twoList as twoList>
						<option value="${twoList.id!}" <#if view.twoParentId==twoList.id>selected</#if>>${twoList.name!}</option>
					</#list>
				</select>
			</div>
		</div>
    ...
    layui.use(['form','jquery','layer','laydate','layedit'],function(){
       var form = layui.form,
        ...
        form.on("submit(modify)",function(data){
            ...
            $.post("${serverPath}/mgn/finance/newTypeThree/modify",data.field,function(res){
