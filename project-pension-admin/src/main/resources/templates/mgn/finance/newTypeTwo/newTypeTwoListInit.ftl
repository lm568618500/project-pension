...
<fieldset class="layui-elem-field">
    ...
    <form class="layui-form">
        ...
        <div class="layui-inline" style="margin-left: 15px">
            <label>二级分类:</label>
            <div class="layui-input-inline">
                <select name="c_parent_two_id" id="c_parent_two_id" lay-verify="required">
                    <option value="-10001">全部</option>
                </select>
            </div>
        </div>
...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					{field:'name', title: '二级分类'},
                    {field:'nameEn', title: '二级分类英文'},
					{field:'time', title: '时间'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/finance/newTypeTwo/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/finance/newTypeTwo/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/finance/newTypeTwo/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/finance/newTypeTwo/newTypeTwoShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/finance/newTypeTwo/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "编辑数据",
                    type : 2,
                    content : "${serverPath}/mgn/finance/newTypeTwo/newTypeTwoEditInit?id=" + data.id,
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "新增数据",
                    type : 2,
                    content : "${serverPath}/mgn/finance/newTypeTwo/newTypeTwoAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
