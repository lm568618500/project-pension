...
<form class="layui-form" style="width:80%;">
		...
		<div class="layui-input-inline">
			<select name="parentOneId" lay-verify="required">
				...
				<#list oneList as oneList>
					<option <#if view.parentOneId == oneList.id>selected</#if> value="${oneList.id!}">${oneList.name!}</option>
				</#list>
			</select>
		</div>
	</div>
	<div class="layui-form-item">
		...
		<div class="layui-input-block">
			<input type="hidden" name="id" lay-verify="title" value="${view.id!}" autocomplete="off" placeholder="请输入id" class="layui-input">
    ...
    layui.use(['form','jquery','layer','laydate','layedit'],function(){
       var form = layui.form,
        ...
        form.on("submit(modify)",function(data){
            ...
            $.post("${serverPath}/mgn/finance/newTypeTwo/modify",data.field,function(res){
