    ...
    <form class="layui-form">
        <div class="layui-inline" style="margin-left: 15px">
            ...
            <div class="layui-input-inline">
                <select name="c_parent_one_id" lay-verify="required">
...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					{field:'nameEn', title: '一级分类英文'},
					{field:'time', title: '时间'},
					{field:'lastupdate', title: '更新时间'},
					{field:'break', title: '备注'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/finance/newTypeOne/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/finance/newTypeOne/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/finance/newTypeOne/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/finance/newTypeOne/newTypeOneShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/finance/newTypeOne/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "编辑数据",
                    type : 2,
                    content : "${serverPath}/mgn/finance/newTypeOne/newTypeOneEditInit?id=" + data.id,
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "新增数据",
                    type : 2,
                    content : "${serverPath}/mgn/finance/newTypeOne/newTypeOneAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
