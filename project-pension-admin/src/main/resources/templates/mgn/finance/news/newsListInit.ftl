    ...
    <form class="layui-form">
        <div class="layui-inline" style="margin-left: 15px">
            ...
            <div class="layui-input-inline">
                <input type="text" name="c_name" lay-verify="title" autocomplete="off" placeholder="请输入新闻名称" class="layui-input">
            </div>
        </div>
        <div class="layui-inline" style="margin-left: 15px">
            <label>一级分类:</label>
        ...
        <div class="layui-inline" style="margin-left: 15px">
            <label>三级分类:</label>
            <div class="layui-input-inline">
                <select name="c_parent_three_id" id="c_parent_three_id" lay-filter="threeSelect">
                    <option value="-10001">全部</option>
        ...
        <div class="layui-inline" style="margin-left: 15px">
            <label>四级分类:</label>
            <div class="layui-input-inline">
                <select name="c_parent_four_id" id="c_parent_four_id">
                    <option value="-10001">全部</option>
...
<script>
    layui.use(['layer','form','table'], function() {
		...
		var cols = [
					...
					{field:'parentThreeName', title: '三级分类'},
					{field:'parentFourName', title: '四级分类'},
					{field:'name', title: '新闻名称'},
					{field:'nameEn', title: '新闻名称(英文)'},
					{field:'time', title: '时间'},
        ...
        t = {
            elem: '#test',
            url:'${serverPath}/mgn/finance/news/getByPage',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'start') {
				layer.confirm("你确定要开启么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/finance/news/start",{"id":data.id},function (res){
                           if(res.result.ret){
				...
				layer.confirm("你确定要关闭么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/finance/news/stop",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
				...
				console.log(obj);
					var addIndex = layer.open({
	                    title : "基础信息",
	                    type : 2,
	                    content : "${serverPath}/mgn/finance/news/newsShowInit?id=" + data.id,
				...
				layer.confirm("你确定要删除么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${serverPath}/mgn/finance/news/del",{"id":data.id},function (res){
                           if(res.result.ret){
                               layer.msg("操作成功",{time: 1000},function(){
                                   table.reload('test', t);
                               });
            ...
            if(obj.event === "editInit"){
                var addIndex = layer.open({
                    title : "编辑数据",
                    type : 2,
                    content : "${serverPath}/mgn/finance/news/newsEditInit?id=" + data.id,
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addInit : function(){
                var addIndex = layer.open({
                    title : "新增数据",
                    type : 2,
                    content : "${serverPath}/mgn/finance/news/newsAddInit",
                    success : function(layero, addIndex){
                        setTimeout(function(){
                            layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        form.on('select(threeSelect)', function(data){
            if(data.value==-10001){
                $("#c_parent_four_id").html("");
                var option1 = $("<option value='-10001'>").text("请选择全部");
                $("#c_parent_four_id").append(option1);
                form.render('select');
                return false;
            }
            $.ajax({
                ...
                success: function(data){
                    var option1 = $("<option value='-10001'>").text("请选择全部");
                    $("#c_parent_four_id").empty();
                    $("#c_parent_four_id").append($("<option value='-10001'>").text("请选择全部"));
