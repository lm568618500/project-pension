<@override name="body">
  ...
  <div class="box-container-fluid page-nav-wrapper">
    <div class="box">
      <div class="title-box">
        <div class="title">${one.nameEn!}</div>
        <p>${one.nameEn!}</p>
      </div>
      <div class="page-nav-box">
        <div class="intruction-caption" onclick="itemToggle(this)" data-status="0">${two.nameEn!}</div>
        <div class="nav">
      ...
      <div class="breadcrumb">
        <a href="/indexEn" class="home"><img src="${PubStaticServer}/static/jw/images/home.png" alt></a>
        <img src="${PubStaticServer}/static/jw/images/icon.png" alt class="icon">
        <a href="${serverPath}/jw/newsEn?oneId=${one.id!}&twoId=0&threeId=0&fourId=0&newId=0">${one.nameEn!}</a>
        <img src="${PubStaticServer}/static/jw/images/icon.png" alt class="icon">
        <span>${two.nameEn!}</span>
      </div>
    </div>
  </div>
  <!-- 二级栏目 end -->
      ...
      <div class="mini-container" id="layer-photos-demo">
        ...
        <div class="page-category-box wow fadeInUp50">
          <div class="inner">
            ...
            <a  <#if threeList.id=threeId>class="on"</#if> href="${serverPath}/jw/newsEn?oneId=${threeList.oneParentId!}&twoId=${threeList.twoParentId!}&threeId=${threeList.id!}&fourId=0&newId=0" >${threeList.nameEn!}</a>
            </#list>
          </div>
        </div>
        <#if fourSize&gt;0>
          <div class="page-cat-box wow fadeInUp50">
            <ul id="seriesConditionType">
              <li>
                <span class="cat">classification：</span>
                <a href="${serverPath}/jw/newsEn?oneId=${oneId!}&twoId=${twoId!}&threeId=${threeId!}&fourId=0&newId=0" data-da="0" <#if fourId==0> class="on"</#if> >ALL</a>
                <#list  fourList as fourList>
                  <a <#if newId &gt;= 0> <#if fourId==fourList.id> class="on"</#if></#if> href="${serverPath}/jw/newsEn?oneId=${fourList.oneParentId!}&twoId=${fourList.twoParentId!}&threeId=${fourList.threeParentId!}&fourId=${fourList.id}&newId=0" data-da="23" >${fourList.nameEn!}</a>
                </#list>
              </li>
            </ul>
          </div>
        ...
        <div class="list">
          <ul id="DivList">
                <#list newsList as newsList>
                  <li>
                    <a href="${serverPath}/jw/newsEn?oneId=${newsList.typeOneId!}&twoId=${newsList.typeTwoId!}&threeId=${newsList.typeThreeId!}&fourId=${newsList.typeFourId!}&newId=${newsList.id!}">
                      <div class="pic"><img src="${newsList.images!}" alt="${newsList.name!}" width="684px" height="370px"></div>
                      <div class="info">
                        <div class="name">${newsList.nameEn!}</div>
                        <div class="desc">
                          ${newsList.describesEn!}
                        </div>
